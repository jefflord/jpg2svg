...
user-level config file + --config file). `config init` writes a commented
TOML template.
"""
...
config_app.command("help")(make_group_help_command("config"))
...
def config_show_command(
    ...
    # PRD 8: the user-level config file (level 3) applies below --config
    # (level 4) and above presets.
...
def _render_toml(preset_name: str | None) -> str:
    ...
    lines: list[str] = [
        ...
        "#",
        "# Precedence (low to high): engine defaults < preset < user-level config",
        "# < this file < CLI options. (User-level: config.toml in the app data dir.)",
